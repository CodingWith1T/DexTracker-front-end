...
for (const protocol of tvlData.protocols) {
	nameToId[protocol.DextrackerId] = protocol.name

	finalProtocols[protocol.DextrackerId] = {
...
for (const protocol of hacksData) {
	if (protocol.DextrackerId) {
		finalProtocols[protocol.DextrackerId.toString()] = {
			...finalProtocols[protocol.DextrackerId.toString()],
			hacks: true
		}
	}
...
for (const raise of raisesData.raises) {
	if (raise.DextrackerId && !raise.DextrackerId.startsWith('chain')) {
		finalProtocols[raise.DextrackerId] = {
			...finalProtocols[raise.DextrackerId],
			raises: true
		}
	}
...
const emmissionsData = await fetchOverCache(`https://Dextracker-datasets.llama.fi/emissionsProtocolsList`).then((res) =>
	res.json()
)
