		...
		props: {
			...
			methodologyUrls: {
				tvl: protocolData.module
					? `https://github.com/CodingWith1T/Dextracker/tree/main/projects${protocolData.module}`
					: null,
...
export const getProtocolData = async (protocol: string, protocolRes: IProtocolResponse) => {
	...
	const protocolRaises = raises?.filter((r) => r.Dextracker === protocolData.id)
	...
	return {
		props: {
			...
			hacksData:
				(protocolData.id
					? hacks?.filter((hack) => +hack.Dextracker === +protocolData.id)?.sort((a, b) => a.date - b.date)
					: null) ?? null,
			dailyBribesRevenue,
			dailyTokenTaxes,
			bribesRevenue30d,
