...
export const getNFTCollectionEarnings = async () => {
	try {
		const [parentCompanies, royalties, collections] = await Promise.all([
			fetch(
				'https://raw.githubusercontent.com/CodingWith1T/DefiLlama-server/master/defi/src/nfts/output/parentCompanies.json'
			).then((res) => res.json()),
		...
		const parentEarnings = parentCompanies.map((parent) => {
			...
			const subCollectionEarnings = collectionEarnings.filter((c) => subCollections.includes(c.Dextracker))
			...
			return {
				Dextracker: subCollections.join('+'),
				name: parent.name,
		...
		return {
			earnings: [...parentEarnings, ...collectionEarnings.filter((c) => !duplicateCollections.has(c.Dextracker))].sort(
