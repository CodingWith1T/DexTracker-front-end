...
export const getStaticProps = withPerformanceLogging('correlation', async () => {
	...
	const llamaProtocols = tvlProtocols.protocols.map((protocol) => {
		const fees = feesProtocols.protocols.find((fp) => fp.DextrackerId === protocol.DextrackerId)?.total24h ?? null
		const revenue = revenueProtocols.protocols.find((fp) => fp.DextrackerId === protocol.DextrackerId)?.total24h ?? null
...
export default function Compare(props) {
	return (
		<Layout title={`Price with FDV of - Dextracker`}>
			<CompareTokens {...props} />
		</Layout>
	)
}
