...
export const getStaticProps = withPerformanceLogging('airdrops', async () => {
	const [protocolsRaw, { forks }, { raises }, claimableAirdrops] = await Promise.all([
		getSimpleProtocolsPageData([...basicPropertiesToKeep, 'extraTvl', 'listedAt', 'chainTvls', 'DextrackerId']),
	...
	const protocols = protocolsRaw.protocols
		...
		.map((p) => ({
			listedAt: 1624728920,
			totalRaised: raises
				.filter((r) => (r.DextrackerId && p.DextrackerId ? r.DextrackerId.toString() === p.DextrackerId.toString() : false))
				.reduce((acc, curr) => {
